...
function Actives ({data, onDelete, onArchive}) {
  let dataActive = useState([]) 
  dataActive = data.filter(x => x.archived === false)
  ...
  return (
    <div className="w-full py-3">
      ...
      {
        dataActive.length < 1 ?
        <div className="w-full text-slate-300 text-center text-sm">Active Notes is Empty</div>
        :
              ...
              return (
                <div key={i} className="flex flex-col justify-between bg-slate-800 rounded-md shadow-sm border border-slate-800 hover:border-sky-500 highlight-white/5 hover:bg-slate-700 p-4">
                  ...
                  <div className="flex justify-between border-2 border-sky-800 border-x-0 border-b-0 mt-6 pt-2">
                    <div onClick={() => onArchive(item.id)}  className="cursor-pointer text-sm text-yellow-700 hover:text-yellow-400 px-2">
                      Archieve
                    </div>
